...
function UpdateDevilChams()
    for _, v in ipairs(game.Workspace:GetChildren()) do
        if v:IsA("Model") and v:FindFirstChild("Handle") then
            pcall(function()
                if DevilFruitESP and string.match(v.Name, "Fruit") then
                    local handle = v.Handle
                    local billboard = handle:FindFirstChild("NameEsp" .. Number)
                    ...
                    local distance = math.floor((player.Character.Head.Position - handle.Position).Magnitude / 3)
                    billboard.TextLabel.Text = string.format("%s\n%d Distance", v.Name, distance)
...
local Window = OrionLib:MakeWindow({Name = "Zenith hub", HidePremium = false, IntroText = "", SaveConfig = true, ConfigFolder = "Zenith hub"})
